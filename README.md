...
## Возможности
- загрузка нескольких файлов (до 15 за раз), лимит 75 MB на файл, серверное имя = UUID
- для изображений:
  - миниатюры превью (`/preview/*`)
  - история загрузок общая для всех пользователей (`/history`)
  - ресайз по фиксированным ширинам (720/1080/1280/1920/2440) по запросу (`/resize`), апскейл разрешён
  - кадрирование (crop) через UI с выбором пропорций (1:1, 2:3, 16:9), кадрирование всегда от неизменённого исходника (`upload-original/*`)
  - Split (склейка) двух изображений 1280 в одно 1280×720 с белой полосой 7px по центру (`/split`)
  - удаление записи из истории и всех связанных файлов (`/delete`)
  - авто-очистка файлов старше 48 часов (на запросах)
